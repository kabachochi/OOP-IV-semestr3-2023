...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            if (type == 1)
            {
                pos_lines = lines[i].move_line();
                pos_circles = circles[i].move_circle();
                pos_rectangles = rectangles[i].move_rectangle();
                pos_triangles = triangles[i].move_triangle();
                pos_ellipses = ellipses[i].move_ellipse();
                sfml_ellipses[i].setPosition(Vector2f(pos_ellipses[0], pos_ellipses[1]));
                pos_rhombs = rhombs[i].move_rectangle();
            }
            else
            {
                angle = angle + 0.02;
                pos_lines = lines[i].rotate_line(angle);
                pos_rectangles = rectangles[i].rotate_rectangle(angle);
                pos_triangles = triangles[i].rotate_triangle(angle);
                pos_ellipses = ellipses[i].rotate_ellipse(angle);
                sfml_ellipses[i].setRadius(Vector2f(pos_ellipses[0], pos_ellipses[1]));
                // sfml_ellipses[i].rotate(angle);
                pos_rhombs = rhombs[i].rotate_rectangle(angle);
            }
            pos_circles = circles[i].move_circle();
